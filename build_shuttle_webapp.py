...
<body>
  ...
  <script>
    ...
    function positionMenuPanel() {{
      ...
      const shellRect = topShell?.getBoundingClientRect();
      // .top-shell uses backdrop-filter, which becomes the containing block
      // for this fixed panel. Convert the button's viewport position to that
      // containing block so the panel corner meets the button corner exactly.
      const rightOffset = Math.max(0, (shellRect?.right ?? window.innerWidth) - rect.right);
      const topOffset = rect.bottom - (shellRect?.top ?? 0);
